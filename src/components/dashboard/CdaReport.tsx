...
function HorizontalBars({
  ...
  return (
    <Card className="rounded-[2rem] border border-slate-200 shadow-none">
  ...
  return (
    <div className="min-h-screen bg-slate-100 print:bg-white">
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-6 py-8 print:px-0 print:py-0">
        <section className="report-card overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-8 border-b border-slate-100 px-8 py-8 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              <div className="flex h-20 w-20 items-center justify-center rounded-[1.5rem] bg-slate-50 p-3">
          ...
          <div className="grid grid-cols-1 gap-5 p-8 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard label="Tot Asset" value={stats.totalAssets.toString()} tone="border-blue-100 bg-blue-50/50" />
            <MetricCard label="Conformità" value={`${stats.conformanceRate}%`} tone="border-emerald-100 bg-emerald-50/50" />
            <MetricCard label="Asset Scaduti" value={stats.expiredAssets.toString()} tone="border-red-100 bg-red-50/50" />
            <MetricCard label="WO In Corso" value={stats.woInProgress.toString()} tone="border-orange-100 bg-orange-50/50" />
          </div>
        </section>

        <section className="grid grid-cols-1 gap-8 xl:grid-cols-2">
          <Card className="report-card rounded-[2rem] border border-slate-200 shadow-sm">
              ...
              {stats.recentWorkOrders.length === 0 ? (
                ...
                <div className="space-y-3">
                  {stats.recentWorkOrders.map((workOrder) => (
                    <div key={workOrder.id} className="rounded-[1.5rem] border border-slate-200 p-4">
